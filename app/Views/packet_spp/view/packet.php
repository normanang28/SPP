...
<?php if(session()->get('level')== 1) { ?>
            <a href="<?= base_url('/home/add_packet/')?>"><button class="btn btn-success"><i class="fa fa-plus"></i> Add</button></a>
<?php }else{} ?>
...
<?php if(session()->get('level')== 1) { ?>
        ...
        <th class="text-center">Packet Name</th>
...
<?php if(session()->get('level')== 1) { ?>
                      ...
                      <td class="text-uppercase text-center"><?php echo $gas->nama_paket?></td>
                      <td class="text-uppercase text-center"><?php echo $gas->harga_paket?></td>
                      <td class="text-capitalize text-center"><?php echo $gas->username?></td>      
                      <td>
<div class="button-container">
                        <a href="<?= base_url('/home/edit_packet/'.$gas->id_paket)?>"><button class="btn btn-warning"><i class="fa fa-edit"></i> </button></a>
                        <a href="<?= base_url('/home/delete_packet/'.$gas->id_paket)?>"><button class="btn btn-danger"><i class="fa fa-trash"></i> </button></a>
</div>
<style>
    .button-container {
    display: flex;
...
function displayDataOnPage(page) {
    ...
    for (let i = startIndex; i < endIndex && i < data.length; i++) {
        ...
        <tr>
            ...
            <td class="text-uppercase text-center text-dark">${gas.nama_paket}</td>
            <td class="text-capitalize text-center text-dark">${gas.harga_paket}</td>
