...
<?php if(session()->get('level')== 1) { ?>
            <a href="<?= base_url('/home/add_teacher/')?>"><button class="btn btn-success"><i class="fa fa-plus"></i> Add</button></a>
<?php }else{} ?>
...
<?php if(session()->get('level')== 1) { ?>
                        ...
                        <div class="col-12 center-column">
                          <a href="<?= base_url('/home/detail_teacher/'.$gas->id_user_guru)?>"><button class="btn btn-info"><i class="fa fa-bars"></i> Details</button></a>
                        </div>
                        </td>
                        <?php }else{} ?>
                        </tr>
...
function displayDataOnPage(page) {
    ...
    for (let i = startIndex; i < endIndex && i < data.length; i++) {
        ...
        <tr>
            ...
            <td class="text-center text-dark">${gas.nik}</td>
            <td class="text-capitalize text-center text-dark">${gas.nama_guru}</td>
            <td class="text-center text-dark">${gas.email_guru}</td>
            <td class="text-capitalize text-center text-dark">${gas.jk_guru}</td>
